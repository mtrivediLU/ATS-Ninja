...
from core.llm import invoke_json, invoke_text, run_concurrently
...
def build_resume_plan(
    *,
    ...
    `llm` writes short prose (the summary); `batch_llm` handles the bulk
    JSON bullet rewrite, which benefits from a larger output budget. Pass
    the same client for both if there is no need to distinguish them.
    """
    batch_llm = batch_llm if batch_llm is not None else llm
    ...
    # The summary and the bullet rewrite are independent LLM calls; running
    # them concurrently instead of back-to-back roughly halves this part of
    # the pipeline's wall-clock time whenever a local model is available.
    results = run_concurrently(
        {
            "summary": lambda: _build_summary(role_identity, top_keywords, jd_profile, profile, years_span, llm),
            "experience": lambda: _select_experience(profile, evidence, jd_profile, batch_llm),
        }
    )
...
def build_cover_letter_plan(resume_plan: ResumePlan, profile: Profile, llm: Any | None = None) -> CoverLetterPlan:
...
def build_answer_plan(
    *,
    ...
    Logistics questions (salary, start date, sponsorship, work mode) are
    answered instantly from resolved contact facts, no LLM needed. Any
    remaining open-ended questions each need one LLM call; those calls are
    independent of each other, so they run concurrently rather than one
    question at a time.
    """
    ...
    for index, question in enumerate(questions):
        ...
        if "salary" in lowered or "compensation" in lowered:
            answers[index] = "My target range is [YOUR RANGE], depending on total compensation, role scope, and work mode."
            placeholders.append("[YOUR RANGE]")
        elif "start" in lowered or "available" in lowered:
            location_clause = f" I am based in {resume_plan.contacts.location}." if resume_plan.contacts.location else ""
            answers[index] = f"I can start on [YOUR START DATE].{location_clause} I can align with the role's work mode."
            ...
            answers[index] = f"{base} I am open to the role's work mode and open to relocation for the right opportunity.".strip()
        else:
            open_questions[index] = question

    if open_questions:
        tasks = {
            str(index): (lambda q=question: _long_answer(q, resume_plan, llm))
            for index, question in open_questions.items()
        }
...
def _build_summary(
    ...
    return _llm_generate(llm, prompt, validate, fallback)
...
def _fallback_summary(
    ...
    return (
        ...
        f"Brings hands-on delivery experience aligned with {domain} needs. "
...
def _rewrite_bullets_batch(
    ...
    """Rewrite every selected bullet in a single LLM call instead of one call per bullet.

    This is the single biggest latency lever in the pipeline: a resume with
    20 selected bullets used to mean 20+ sequential round trips to the local
    model. Batching cuts that to one call, with the same per-bullet
    groundedness checks applied to each item in the response, falling back
    to that item's untouched original on any failure.
    """
    ...
    rewritten: list[str] = []
    for original, candidate in zip(bullets, data):
...
def _build_cover_letter_body(
    *,
    ...
    proof_text = "\n".join(f"- {point}" for point in proof_points) or "- No specific bullets were available; speak generally about the candidate's background."
    ramp_line = (
    ...
    raw = _llm_generate(llm, prompt, validate, "")
    if not raw:
        return fallback
...
def _fallback_cover_letter_body(
    *,
    ...
    first_proof = proof_points[0] if proof_points else "The candidate has built software and data systems across multiple roles."
    second_proof = (
        proof_points[1] if len(proof_points) > 1 else "The candidate has combined technical delivery with stakeholder-facing work across roles."
    )
    third_proof = proof_points[2] if len(proof_points) > 2 else "The candidate's background spans multiple environments and team sizes."
    ramp = ""
    if needs_fast_ramp:
...
def _long_answer(question: str, resume_plan: ResumePlan, llm: Any | None) -> str:
    fallback = _fallback_long_answer(question, resume_plan)
    if llm is None:
        return fallback

    proof = resume_plan.experience[0].bullets[0] if resume_plan.experience and resume_plan.experience[0].bullets else resume_plan.summary
    prompt = (
...
def _llm_generate(
    ...
    """Call the LLM, validate the result, retry once with feedback, else fall back."""
    candidate = _clean_llm_line(invoke_text(llm, prompt))
    attempt = 0
