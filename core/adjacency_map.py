...
This is candidate-agnostic: it only says which tools sit in the same
functional category (data warehouse, orchestration, BI, etc). The evidence
engine pairs a JD's missing tool with whatever real tool the candidate's own
profile shows in the same category, so the phrasing is always grounded in
something the candidate actually did, never invented.
"""
...
TOOL_CATEGORIES: dict[str, dict[str, object]] = {
    "cloud_data_warehouse": {
        ...
        "tools": [
            ...
            "sql server", "azure synapse", "synapse", "databricks sql",
            "oracle", "teradata",
        ],
    },
    "orchestration": {
        ...
        "tools": [
            "airflow", "prefect", "dagster", "dbt", "azure data factory",
            "luigi", "aws glue", "glue", "informatica", "ssis",
        ],
    },
    "big_data_processing": {
    ...
    "bi_tool": {
        ...
        "tools": ["tableau", "power bi", "looker", "qlik", "sigma", "metabase", "looker studio"],
    },
    "cloud_provider": {
    ...
    "llm_framework": {
        ...
        "tools": [
            ...
            "vector database", "pinecone", "weaviate", "chroma", "openai api",
    ...
    "backend_framework": {
        ...
        "tools": [
            ...
            "nestjs", ".net", "c#", "asp.net", "hibernate",
        ],
    },
    "rpa": {
    ...
    "ml_framework": {
        ...
        "tools": [
            "scikit-learn", "tensorflow", "pytorch", "keras", "random forest",
...
def find_category(keyword: str) -> tuple[str, str, list[str]] | None:
    ...
    for category, data in TOOL_CATEGORIES.items():
        ...
        if normalized in tools:
            return category, str(data["label"]), list(tools)  # type: ignore[arg-type]
    return None


def category_tools(category: str) -> list[str]:
    data = TOOL_CATEGORIES.get(category)
    return list(data["tools"]) if data else []  # type: ignore[arg-type,return-value]
