...
from core.latex_renderer import parse_resume_sections
from core.models import PipelineResult, Profile
...
def _validate_resume_completeness(sections: dict[str, Any], profile: Profile) -> list[str]:
    ...
    if output_experience_count < source_experience_count:
        errors.append(f"completeness: resume has {output_experience_count} experience entries, source has {source_experience_count}")
    if output_bullet_count < source_bullet_count:
        errors.append(f"completeness: resume has {output_bullet_count} experience bullets, source has {source_bullet_count}")
