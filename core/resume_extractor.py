...
# asked to point at line numbers for bullets instead of retyping them: that
# is both faster (no need to decode the bullet text a second time) and
# strictly more grounded (the resolved text is a guaranteed verbatim slice
# of the source, not the model's reproduction of it).
...
def extract_profile(resume_text: str, llm: Any | None = None) -> Profile:
    ...
    deterministic heuristic parser when no LLM is available or the LLM
...
def _clean_experiences(raw_entries: list[Any], source_text: str) -> list[Experience]:
    ...
    for entry in raw_entries:
        ...
        if not company and not title:
            continue
        # Guard against fabricated employers: the company name (or a
        # meaningful chunk of it) must actually appear in the source resume.
...
# --------------------------------------------------------------------------
# Deterministic fallback parser (used when no LLM is reachable, or the LLM
# output fails the usability check above).
# --------------------------------------------------------------------------
...
_SECTION_KEYWORDS = {
    ...
    "certifications": {"certifications", "certificates", "licenses", "licenses and certifications", "licenses & certifications"},
}
